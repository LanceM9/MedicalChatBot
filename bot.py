...
# Train chatbot from Real Python
trainer.train([
    "Hi",
    "Welcome, friend",
])
...
textHistory = tk.Listbox(userInterface, height=25,
...
textbox = tk.Entry(textboxSendPackage, font=("Helvetica", 18), width=95)
textbox.pack(side=tk.LEFT, fill=tk.X)

send = tk.Button(textboxSendPackage, text='Send',
                 font=('Helvetica', 18), command=text)
send.pack(side=tk.RIGHT, padx=10, pady=10)

export = tk.Button(textboxSendPackage, text='Export',
                   font=('Helvetica', 18), command=export)
